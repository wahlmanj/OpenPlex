...
script AppDelegate
        ...
        tell application "Finder"
            if (exists folder "Applications:PlexConnect:update:OSX" of the startup disk) then
                ...
                if x is equal to "Already up-to-date." then
                    display dialog "No updates avaliable..." & y with title "PlexConnect Status"
    ...
    on buttonhandlerclone_(sender)
        ...
        tell application "Finder"
            ...
            if not (exists file "Applications:PlexConnect:PlexConnect.log" of the startup disk) then
                display dialog "PIL is not installed or theme is experiencing issues, attempting repairs..." with title "OpenPlex Status"
                ...
                if not (exists file "Applications:PlexConnect:PlexConnect.log" of the startup disk) then
                    display dialog "Repair unsuccessful, PIL is not installed or theme is experiencing issues"
                end if
            end if
        end tell
    ...
    on buttonhandlerdeleteATVSettings_(sender)
        tell application "Finder"
            if (exists file "Applications:PlexConnect:ATVSettings.cfg" of the startup disk) then
                display dialog "Exit and/or Open aTV hijack to load default PlexConnect Settings..." with title "OpenPlex Status"
                delay 0
